...
namespace sylar {
    namespace http {
        DealfileServlet::DealfileServlet(const std::string& path)
            :Servlet("DealfileServlet"), m_path(path) {}

        //读取配置信息
        void DealfileServlet::read_cfg() {
            //读取mysql数据库配置信息
        ...
        int32_t DealfileServlet::handle(sylar::http::HttpRequest::ptr request
            ...
            if (len <= 0) {
                ...
                {
                    share_file(user, md5, filename, response, session);
                } else if (strcmp(cmd, "del") == 0) //删除文件
                {
                    del_file(user, md5, filename, response, session);
                } else if (strcmp(cmd, "pv") == 0) //文件下载标志处理
                {
                    pv_file(user, md5, filename, response, session);
                }
        ...
        //解析的json包
        int DealfileServlet::get_json_info(char* buf, char* user, char* token, char* md5, char* filename) {
            int ret = 0;
...
//分享文件
        int DealfileServlet::share_file(char* user, char* md5, char* filename
            , sylar::http::HttpResponse::ptr response
            , sylar::http::HttpSession::ptr session) {
            /*
        ...
        int DealfileServlet::remove_file_from_storage(char* fileid) {
            int ret = 0;
...
//删除文件
        int DealfileServlet::del_file(char* user, char* md5, char* filename
            , sylar::http::HttpResponse::ptr response
            , sylar::http::HttpSession::ptr session) {
            /*
            a)先判断此文件是否已经分享
            b)判断集合有没有这个文件，如果有，说明别人已经分享此文件(redis操作)
            c)如果集合没有此元素，可能因为redis中没有记录，再从mysql中的sharefile表查询，如果mysql也没有，说明真没有(mysql操作)
        ...
        */
        ...
        //文件下载标志处理
        int DealfileServlet::pv_file(char* user, char* md5, char* filename
            , sylar::http::HttpResponse::ptr response
            , sylar::http::HttpSession::ptr session) {
            int ret = 0;
            char sql_cmd[SQL_MAX_LEN] = { 0 };
