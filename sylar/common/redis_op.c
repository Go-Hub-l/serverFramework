...
#define REDIS_DEBUG_LOG_MODULE "cgi"
#define REDIS_DEBUG_LOG_PROC "redis_debug"
...
int rop_zset_zrevrange(redisContext *conn, char *key, int from_pos, int end_pos, RVALUES values, int *get_num)
{
    ...
    *get_num = max_count; //得到结果value的个数
    //LOG(REDIS_DEBUG_LOG_MODULE, REDIS_DEBUG_LOG_PROC, "=====\n");
    ...
    for (i = 0; i < max_count; ++i)
    {
        ...
        LOG(REDIS_DEBUG_LOG_MODULE, REDIS_DEBUG_LOG_PROC, "=====%s\n", values[i]);
    }
